...
bool StringEncryptionPass::do_StrEnc(Module &M, ModuleAnalysisManager &AM)
{
    ...
    for (GlobalVariable &GV : M.globals())
    {
        ...
        {
            if (CDS->isCString())//以null字节结尾的字符串
            {
                ...
                GlobalVariable *DecGV = new GlobalVariable(M, CDS->getType(), false, GlobalValue::PrivateLinkage, ZeroInit, "dec" + Twine::utohexstr(Entry->ID) + GV.getName());
                GlobalVariable *DecStatus = new GlobalVariable(M, Type::getInt32Ty(Ctx), false, GlobalValue::PrivateLinkage, Zero, "dec_status_" + Twine::utohexstr(Entry->ID) + GV.getName());
    ...
    for (CSPEntry *Entry : ConstantStringPool)
    {
        ...
        std::string str(Entry->Data.begin(), Entry->Data.end());
    ...
    for (GlobalVariable *GV : ConstantStringUsers)
    {
        if (isValidToEncrypt(GV))
        {
            ...
            GlobalVariable *DecGV = new GlobalVariable(M, EltType, false, GlobalValue::PrivateLinkage, ZeroInit, "dec_" + GV->getName());
            DecGV->setAlignment(MaybeAlign(GV->getAlignment()));
                            ...
                            PHI->getIncomingValue(i)))
                    {
                        ...
                        { // GV is a constant string
                            ...
                            else
                            {
                                ...
                                Value *OutBuf = IRB.CreatePointerCast(
